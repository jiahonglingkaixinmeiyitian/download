...
- (instancetype)init{
    self = [super init];
    if (self) {
        NSURLSessionConfiguration *configuration = [NSURLSessionConfiguration backgroundSessionConfigurationWithIdentifier:@"com123.bundleiD.TES"];
        //设置请求超时为10秒钟
    ...
    if (downLoadHistoryData.length > 0 ) {
        ...
        downloadTask = [self.manager downloadTaskWithResumeData:downLoadHistoryData progress:^(NSProgress * _Nonnull downloadProgress) {
            if (progress) {
                ...
                NSLog(@"旧任务：%F",(1.0 * downloadProgress.completedUnitCount / downloadProgress.totalUnitCount));
            }
        ...
        downloadTask = [self.manager    downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            NSLog(@"新任务：%F",(1.0 * downloadProgress.completedUnitCount / downloadProgress.totalUnitCount));
        } destination:^NSURL * _Nonnull(NSURL * _Nonnull targetPath, NSURLResponse * _Nonnull response) {
            return localUrl;
